...
import {View, StyleSheet, Pressable, ViewStyle, StyleProp, Text} from 'react-native';
import Animated, {
  useAnimatedStyle,
  useSharedValue,
  withTiming,
    ...
    <View style={[styles.container, containerStyle]}>
      {showButton ? (
        <Pressable onPress={handleOnPress}>
          <FontAwsome5 name="running" color={colors.CONSTRACT} size={24} />
        </Pressable>
      ) : null}
      <Animated.View style={[styles.buttons, widthStyle]}>
        {speedRates.map(item => {
          return <Selector value={item} key={item} active={activeRate === item} onPress={()=> onPress && onPress(+item)} />;
        })}
      </Animated.View>
    </View>
  );
